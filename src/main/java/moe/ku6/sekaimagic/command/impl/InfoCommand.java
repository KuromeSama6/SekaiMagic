...
public class InfoCommand implements ICommand<InfoCommand.Params> {
    ...
    public void HandleInternal(Params args) throws Exception {
        var pkg = SekaiDataManager.getInstance().getPackages().get(Integer.parseInt(args.id));
        if (pkg == null) {
        ...
        log.info(Ansi.ansi().fgGreen().a("* Do `fingers -g %d expert` to generate fingers chart.".formatted(pkg.getId())).reset().toString());
